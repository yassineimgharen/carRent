...
router.post("/", auth, adminOnly, (req, res) => {
  const { name, brand, category, price_per_day, image_url, description, seats, transmission, fuel_type, is_available } = req.body;
  if (!name || !brand || !category || !price_per_day) return res.status(400).json({ error: "Missing required fields" });
  const result = db.prepare(
    "INSERT INTO cars (name, brand, category, price_per_day, image_url, description, seats, transmission, fuel_type, is_available) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
  ).run(name, brand, category, price_per_day, image_url || null, description || null, seats || 5, transmission || "Automatic", fuel_type || "Gasoline", is_available !== false ? 1 : 0);
...
router.put("/:id", auth, adminOnly, (req, res) => {
  ...
  const { name, brand, category, price_per_day, image_url, description, seats, transmission, fuel_type, is_available } = req.body;
  db.prepare(
    "UPDATE cars SET name=?, brand=?, category=?, price_per_day=?, image_url=?, description=?, seats=?, transmission=?, fuel_type=?, is_available=?, updated_at=datetime('now') WHERE id=?"
  ).run(name ?? car.name, brand ?? car.brand, category ?? car.category, price_per_day ?? car.price_per_day, image_url ?? car.image_url, description ?? car.description, seats ?? car.seats, transmission ?? car.transmission, fuel_type ?? car.fuel_type, is_available !== undefined ? (is_available ? 1 : 0) : car.is_available, req.params.id);
