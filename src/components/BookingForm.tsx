...
const BookingForm = ({ car }: { car: Car }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-card p-6 space-y-5">
      <h3 className="font-display text-lg font-semibold">{t('booking.title')}</h3>
      ...
      <Button type="submit" className="w-full rounded-full font-display" size="lg" disabled={loading || !Number(car.is_available)}>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 h-11 rounded-full border border-border hover:border-[#25D366] bg-background hover:bg-[#25D366]/5 transition-all group"
      >
        <div className="h-6 w-6 rounded-full bg-[#25D366] flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        <span className="font-display text-sm">{t('whatsapp.contact')}</span>
      </button>
    </form>
  );
};
